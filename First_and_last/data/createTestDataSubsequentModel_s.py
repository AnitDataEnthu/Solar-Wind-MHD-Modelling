...
def getIntrestingFrames(data,frame_start,frame_end):
    ...
    for file in range(data.shape[0]):
        ...
        short_data[file,5:10]=data[file,frame_end :frame_end + 5,:,:,:]  # tf.v1
...
def create_test_data(frame_start,frame_end,valid_data_paths,test_sample=1):
    # 'clips', 'dims', 'input_raw_data'
    # path=os.getcwd()+"/data/sampleData/"
    # sequence_length=10
    # start=file_start_train
    # end=file_end_train
    # clips_train,dims_train,input_raw_data_train=createData(test_sample,test_sample+1,sequence_length,path,frame_start,frame_end)
    ...
    np.savez(valid_data_location,clips=clips_valid,dims=dims_valid,
             input_raw_data=input_raw_data_valid)  # print("Load arrays from the 'PSI-valid.npz' file:")  # with np.load(valid_data_location) as data:  #     x2 = data['clips']  #     y2 = data['dims']  #     z2 = data['input_raw_data']  #     print(x2.shape)  #     print(y2.shape)  #     print(z2.shape)
