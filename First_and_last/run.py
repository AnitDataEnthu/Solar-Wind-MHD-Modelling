...
import os
import numpy as np
from First_and_last.src.data_provider import datasets_factory
from First_and_last.src.models.model_factory import Model
import First_and_last.src.trainer as trainer
from First_and_last.src.utils import preprocess
import tensorflow as tf
from First_and_last.data.createSubsequentFrames import create_train_data
from First_and_last.data.createTestDataSubsequentModel_s import create_test_data
from generateSSIM_graph import generatecsv
# -----------------------------------------------------------------------------
FLAGS = tf.app.flags.FLAGS
def main(testSample,recombination):
#def main(_):
    del_all_flags(tf.flags.FLAGS)
    FLAGS = tf.app.flags

    FLAGS.DEFINE_string('train_data_paths', 'First_and_last/data/PSI/PSI-train.npz', 'train data paths.')
    FLAGS.DEFINE_string('valid_data_paths', 'First_and_last/data/PSI/PSI-valid.npz', 'validation data paths.')
    FLAGS.DEFINE_string('save_dir', 'First_and_last/checkpoints/_mnist_e3d_lstm/', 'dir to store trained net.')
    FLAGS.DEFINE_string('gen_frm_dir', 'First_and_last/results/_mnist_e3d_lstm/', 'dir to store result.')
    ...
    tf.app.flags.DEFINE_boolean('is_training', False, 'training or testing') #test
    ...
    #FLAGS.DEFINE_integer('batch_size', 8, 'batch size for training.') #test
    ...
    #################### TESTING ########################
    tf.app.flags.DEFINE_boolean('is_PSI_testing', True, 'training or testing')
    FLAGS.DEFINE_string('pretrained_model', 'model.ckpt-87000', '.ckpt file to initialize from.')
    tf.app.flags.DEFINE_boolean('integrate_Yang_Model', False , 'get sample from Yang Model')
    FLAGS.DEFINE_integer('PSI_patch_size', 64, 'for recomibing the images')
    FLAGS.DEFINE_integer('batch_size', 4, 'batch size for training.')
    FLAGS.DEFINE_integer('testSample',1, 'test Sample')
    FLAGS.DEFINE_string('recombination', 'Max Pooling', 'Recombination technique to use')
    ...
    if FLAGS.is_training:
      create_train_data(0,136,"/data/PSI/PSI-train.npz","/data/PSI/PSI-valid.npz")
      train_wrapper(model)
    elif FLAGS.is_PSI_testing:
      generatecsv(testSample)
      callconfig(testSample,recombination)
      create_test_data(0,136,FLAGS.valid_data_paths,FLAGS.testSample)
...
def callconfig(testSample,recombination):
...
def del_all_flags(FLAGS):
    ...
    print("ALL THE FLAGS HAVE BEEN CLEARED!!")
