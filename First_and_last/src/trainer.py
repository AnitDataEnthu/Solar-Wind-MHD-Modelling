...
import First_and_last.combine as combine
import pandas as pd
...
def PSI_test(model,test_input_handle,configs,save_name,size):
  ...
  real_input_flag_zero=np.zeros((configs.batch_size,configs.input_length - 1,configs.img_width // configs.patch_size,
  configs.img_width // configs.patch_size,configs.patch_size ** 2 * configs.img_channel))
  ...
  combine.savesample("",configs.gen_frm_dir,(size,size),configs.recombination
                     )
