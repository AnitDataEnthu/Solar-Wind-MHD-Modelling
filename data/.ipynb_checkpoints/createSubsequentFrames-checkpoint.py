...
def getIntrestingFrames(data,frame_start,frame_end):
    ...
    short_data = np.zeros((data.shape[0],10,128,111,1))           #tf.v1
...
def getIntrestingFrames_susequent_frames(data,samp_no):
    ...
    data_np=data_np.reshape(27,10,128,111,1)
    data_np_fin=data[samp_no,131:141,:,:,:]
    data_np=np.append(data_np,data_np_fin)
    data_np=data_np.reshape(28,10,128,111,1)
    return data_np
...
def createData(start,end,sequence_length,path,frame_start,frame_end):
    data_1=readfile_br(path,start,end) #read the data with the given count 
...
def create_train_data(frame_start,frame_end,train_data_paths,valid_data_paths,file_start_train=0,file_end_train=45,file_start_valid=45,file_end_valid=48):
